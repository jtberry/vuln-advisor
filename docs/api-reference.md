...
!!! tip "Interactive API docs"
    When the server is running, visit `http://localhost:8000/docs` for Swagger UI or `http://localhost:8000/redoc` for Redoc - both are auto-generated from the FastAPI source.
...
No authentication or rate limit on this endpoint.
